...
def test_baseline_all_compliant():
    result = evaluate_file("data/synthetic_logs/scenario_baseline.csv")
    assert result["verdict_counts"]["COMPLIANT"] == 200
    assert result["blocked_count"] == 0


def test_severe_has_blocked():
    result = evaluate_file("data/synthetic_logs/scenario_severe.csv")
    assert result["blocked_count"] > 0
    assert result["average_score"] < 60.0


def test_scores_decrease_across_scenarios():
    baseline = evaluate_file("data/synthetic_logs/scenario_baseline.csv")
    drift = evaluate_file("data/synthetic_logs/scenario_context_drift.csv")
    mixed = evaluate_file("data/synthetic_logs/scenario_mixed_failure.csv")
    severe = evaluate_file("data/synthetic_logs/scenario_severe.csv")
